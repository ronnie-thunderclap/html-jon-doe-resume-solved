...
- [x] Set the required `src` and important `alt` image attributes.
- [x] Set a relative/absolute image `src` attribute value.
- [x] Create a paragraph of text with the `p` tag.
...
- [x] Create a table with the `table` tag.
- [x] Utilize `th` tag to create a table header cell.
- [x] Utilize `td` tag to create a table data cell.
- [x] Set a link with the `a` tag.
- [x] Utilize the anchor `target` attribute to open the destination resource it in a new tab.
- [x] Set the link that starts the default email client by a click with the `mailto:` `href` attribute value.
- [x] Add global attributes to HTML elements.
- [x] Utilize the `link` tag to connect a css file.
- [x] Utilize the `script` tag to connect a js file.
